...
if args.INPUT_MAP == 'r2s':
    ...
    R2P_NET_WEIGHT_NAME = args.R2P_NET_CHECKPOINT_PATH + args.R2P_NET_CHECKPOINT_FILE
    R2P_NET_WEIGHT = torch.load(R2P_NET_WEIGHT_NAME)

    multi_gpu_used = 0
    new_state_dict = OrderedDict()
    for name in R2P_NET_WEIGHT['state_dict']:
        if name[:6] != 'module':
            break
        else:
            multi_gpu_used = 1
            new_name = name[7:]
            new_state_dict[new_name] = R2P_NET_WEIGHT['state_dict'][name]

    if multi_gpu_used == 0:
        r2p_model.load_state_dict(R2P_NET_WEIGHT['state_dict'])
    elif multi_gpu_used == 1:
        logger.info(f'R2PRIMEnet: multi GPU were used')
        r2p_model.load_state_dict(new_state_dict)
